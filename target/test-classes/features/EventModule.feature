  ...
  Feature: Event Module UI Tests
    ...
    @EventPageTestCase1
    Scenario: Test Case 1 - User should view create event form if
    click the Create Event button on dashboard
    ...
    the edit button all event fields must be filled according to editing event
      ...
      Then user checks "<Event Name>" "<Event Description>" "<Event Date>" and participants whether according to editing event
      Examples:
        | Event Name   |Event Description   | Event Date|
        | Welcome 2023 |New Year Celebration| 31/12/2022|

    @EventPageTestCase7
      Scenario Outline: Test Case 7 – User should be able to update event if required fields filled
      and redirect to dashboard page with update successful message
        Given user goes to url
        When user enters valid username and valid password
        Then user clicks create event button and sees create event form
        And user fills "<Event Name>" "<Event Description>" and Event Date fields
        Then user fills participants informations for 4 participants and click the Create New Event button
        And user clicks edit event button
        Then user updates "<New Event Description>" and delete the third(3) participant and add a new participant
        And user clicks update event button and redirect to dashboard page and sees "Event updated successfully" message

        Examples:
          | Event Name   |Event Description   | New Event Description|
          | Welcome 2023 |New Year Celebration| Happy New Year Celebration|